...
def get_db_connection():
    # Retry logic is handled in the calling functions or init_db
    # This just attempts one connection
    try:
    ...
    try:
        ...
        try:
            cur.execute("SELECT id, name, ip, port, username, password, type, 'PASSWORD', '' FROM devices")
             ...
             print("Fetch failed: Table not ready")
             conn.close()
             return
...
def get_snapshot():
    conn = get_db_connection()
    if not conn:
        return jsonify({'nodes': [], 'links': [], 'alarms': []})
